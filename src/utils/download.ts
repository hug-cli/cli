...
export async function getHTML(url: string): Promise<[string, Error]> {
    try {
...
export const downloadFromGitHub = () => {
    const URLPattern = new RegExp(
        `<a class="js-navigation-open ([^"]*)" title="(.*?)" data-pjax="#repo-content-pjax-container" href="(.*?)">`
    )
    const RepoPattern = new RegExp(`(/.*?/.*?/)blob/(.*$)`)
}
